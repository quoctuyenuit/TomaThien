...
class QRScannerViewController: UIViewController, QRScannerViewProtocol {
    ...
    @objc private func applicationWillTerminate(_ application: UIApplication) {
    ...
    @objc private func doneTapped(_ sender: UIBarButtonItem) {
        self.sendServer(from: self.results)
        self.results.removeAll()
//        let toast = ToastViewController.showToast(text: "Lưu thành công")
//        toast.modalPresentationStyle = .overFullScreen
//        self.present(toast, animated: false, completion: nil)
    }
}
...
//            let path = "\(ServerReferncePath.registationList.rawValue)/\(formatted.string(from: date))/\(key)"
            ...
            ServerServices.sharedInstance.pushData(key: key,
                ...
                if let error = error {
                    ...
                    print("Push data successful with key = \(key)")
                }
            })
        }
    }
